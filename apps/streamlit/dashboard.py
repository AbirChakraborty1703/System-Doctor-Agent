...
def main() -> None:
    ...
    st.markdown(
        ...
        <script>
            // Comprehensive console suppression for production environment
            (function() {
                const originalWarn = console.warn;
                const originalError = console.error;
                ...
                // Suppress errors related to tracking/privacy
                console.error = function(...args) {
                    const message = args.join(' ');
                    for (const pattern of suppressPatterns) {
                        if (message.includes(pattern)) return;
                ...
                // Fix iframe sandbox attribute issue by patching DOM creation
                const originalCreateElement = document.createElement;
                document.createElement = function(tagName) {
                    const element = originalCreateElement.call(document, tagName);
                    if (tagName.toLowerCase() === 'iframe') {
                        // Ensure iframes don't have conflicting sandbox attributes
                        const setSandboxAttr = function() {
                            if (element.hasAttribute('sandbox')) {
                                ...
                                if (sandbox.includes('allow-scripts') && sandbox.includes('allow-same-origin')) {
                                    // Replace with safe combination
                                    element.setAttribute('sandbox', 'allow-scripts');
                                }
                            }
                        };
                        setTimeout(setSandboxAttr, 0);
                    }
                    return element;
                };
                
                // Fix popper.js modifier warning
                if (window.Popper) {
                    const original = window.Popper.createPopper || window.Popper;
                    window.Popper.createPopper = function(reference, popper, options) {
                        if (options && options.modifiers) {
                ...
                window.onerror = function(msg, url, lineNo, columnNo, error) {
                    if (msg && msg.includes && (msg.includes('WebSocket') || msg.includes('ws://'))) {
                        return true; // Suppress WebSocket errors
                    }
